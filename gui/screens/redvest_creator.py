...
class RedvestCreatorScreen(ttk.Frame):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.parent_entry = LabledEntry(self, title='Enter Parent Folder', label='Parent Folder', defaultEntry=self.options['parent-name'])
        self.parent_entry.grid(row=0, column=0, columnspan=2, sticky=(N, S, E, W), padx=20, pady=10)

        # Folder name selection
        self.folder_entry = LabledEntry(self, title='Enter New Folder Name', label='New Folder Name', defaultEntry=self.options['folder-name'])
